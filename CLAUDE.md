...
- **运行平台**：Cloudflare Pages + Pages Functions（Workers）+ KV
- **数据存储**：GitHub Contents API——书签数据存为仓库内单个 JSON 文件（`data/data.json`），KV 只做缓存
...
> 本地开发依赖 `wrangler.toml`（在 `.gitignore` 中），首次需自行填入 KV 资源 ID 与 GitHub 配置（`GITHUB_REPO` / `GITHUB_TOKEN`）。未配置 `GITHUB_REPO` 时进入离线模式，数据仅通过 KV 缓存读写。
...
| 名称 | 作用 | 何时改 |
| :--- | :--- | :--- |
| `DATA_VERSION` | 数据文件版本号（`github-data-store.js`） | 数据结构不兼容时 |
...
- 入口：`functions/lib/github-data-store.js`
- 全部数据（分类 / 书签 / 待审核 / 设置）存为仓库内单个 JSON 文件，通过 GitHub Contents API 读写
- **读**用 `loadData(env)`（优先 KV 缓存）；**写**必须先 `readFromGithub(env)` 拿最新 `sha`，再改数据并以 `saveData(env, data, sha)` 写回，避免并发覆盖
- 未配置 `GITHUB_REPO` 时进入**离线模式**：KV 缓存是唯一数据源，`saveData` 只更新缓存，便于本地 / 测试
- 写操作需要 `GITHUB_TOKEN`（Secret 配置）

### 3. 首页是 SSR + KV HTML 缓存（不是静态 HTML）

`functions/index.js` 才是首页 handler。`public/index.html` 只是模板，里面散落着 `{{PLACEHOLDER}}`，由 `index.js` 用 GitHub 数据存储的内容、settings、菜单 / 卡片渲染器（`functions/lib/{card,menu}-renderer.js`）填充。
...
`functions/api/settings.js` 写设置 → 通过 `saveData` 刷新 KV 数据缓存 + 标记 home cache dirty；`functions/index.js` 读取时优先走 `loadData`（KV 缓存），未命中再查 GitHub 并回填缓存。

### 6. 私密书签 / 分类（`is_private`）

- 读取时按 `is_private` 过滤：未认证时过滤掉 `is_private` 的书签 / 分类
...
├── constants.js          # HOME_CACHE_VERSION / FONT_MAP
├── index.js              # 首页 SSR 入口
├── admin/                # /admin、/admin/login、/admin/logout
├── api/                  # REST API（categories/, config/, pending/, cache/, settings.js, ai-chat.js, wallpaper.js …）
└── lib/                  # 共用：card-renderer, menu-renderer, github-data-store, settings-parser, utils, wallpaper-fetcher
...
wrangler.toml             # 已 gitignore，本地需自填 KV id 与 GitHub 配置
```

## 写代码的几条硬性约束

- **修改 `public/css/tailwind.css` 必须 `npm run build:css`** 否则上线无样式
- **任何写数据的接口都要 `markHomeCacheDirty(env, scope)`**，否则首页 KV 缓存会一直返回旧 HTML
...
| 绑定名 | 类型 | 必需 |
| :--- | :--- | :--- |
| `NAV_AUTH` | KV（存 session / CSRF / 限流计数器 / 数据缓存 / home HTML 缓存） | ✅ |

环境变量 / 机密：
- `GITHUB_REPO`（变量）：数据仓库，形如 `owner/repo`；不填则默认写当前仓库 `data/data.json` 并进入离线模式
- `GITHUB_DATA_PATH`（变量）：数据文件路径，默认 `data/data.json`
- `GITHUB_TOKEN`（机密）：写书签数据必需的 token
- 可选：`ENABLE_PUBLIC_SUBMISSION`、`SITE_NAME`、`SITE_DESCRIPTION`、`FOOTER_TEXT`、`ICON_API`、`AI_REQUEST_DELAY`
